        ...
        <div class="col-10 d-flex gap-4 flex-column">
            <form method="post" action="<?= $link->url('record.save') ?>">
                ...
                <div class="input-group has-validation mb-3">
                    <input type="text" class="form-control" name="nazov_discipliny" id="nazov_discipliny" required minlength="2" value="<?= htmlspecialchars(@$record?->getNazovDiscipliny() ?? '') ?>">
                </div>

                <label for="dosiahnuty_vykon" class="form-label fw-bold">Dosiahnutý výkon</label>
                <div class="input-group has-validation mb-3">
                    <input type="text" class="form-control" name="dosiahnuty_vykon" id="dosiahnuty_vykon" value="<?= htmlspecialchars(@$record?->getDosiahnutyVykon() ?? '') ?>">
                </div>

                <label for="datum_vykonu" class="form-label fw-bold">Dátum výkonu</label>
                <div class="input-group has-validation mb-3">
                    <input type="date" class="form-control" name="datum_vykonu" id="datum_vykonu" value="<?= @($record?->getDatumVykonu() ? date('Y-m-d', strtotime($record->getDatumVykonu())) : '') ?>">
                </div>
